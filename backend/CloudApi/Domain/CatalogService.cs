...
public sealed class CatalogService(SqlConnectionFactory connections)
{
    public async Task<CategoryDto> CreateCategoryAsync(ActorContext actor, CreateCategoryRequest input, CancellationToken cancellationToken)
    {
        ...
        var id = Guid.NewGuid();
        await using var command = new SqlCommand("INSERT INTO Categories (Id,OrganizationId,Name) VALUES (@id,@org,@name);", connection);
        command.Parameters.AddWithValue("id", id); command.Parameters.AddWithValue("org", actor.OrganizationId); command.Parameters.AddWithValue("name", input.Name.Trim());
        await command.ExecuteNonQueryAsync(cancellationToken);
        return new(id, input.Name.Trim());
    }
    ...
    public async Task<CatalogProductDto> CreateProductAsync(ActorContext actor, CreateProductRequest input, CancellationToken cancellationToken)
    {
        ...
        var id = Guid.NewGuid();
        await using (var product = new SqlCommand("INSERT INTO Products (Id,OrganizationId,CategoryId,TaxRuleId,PreparationStationId,Sku,Name,Unit,ProductType,HsnCode,GstRate) VALUES (@id,@org,@category,@tax,@station,@sku,@name,@unit,@type,@hsn,@gst); INSERT INTO ProductPrices (Id,OrganizationId,ProductId,LocationId,Price) VALUES (NEWID(),@org,@id,@location,@price); INSERT INTO InventoryBalances (OrganizationId,LocationId,ProductId,OnHand,ReorderLevel) VALUES (@org,@location,@id,0,0);", connection, transaction))
        {
        ...
        return new(id, input.Sku.Trim(), input.Name.Trim(), input.CategoryId, input.Price, input.Unit.Trim(), input.ProductType.Trim().ToUpperInvariant(), true);
    }
