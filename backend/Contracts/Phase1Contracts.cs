...
public sealed record BusinessLocationDto(string BusinessName, string LocationName, string? GstNumber);
public sealed record PosBootstrap(IReadOnlyList<CategoryDto> Categories, IReadOnlyList<ProductDto> Products, IReadOnlyList<ModifierGroupDto> ModifierGroups, IReadOnlyList<ModifierDto> Modifiers, IReadOnlyList<TaxRuleDto> TaxRules, IReadOnlyList<PreparationStationDto> PreparationStations, Guid LocationId, string Currency, BusinessLocationDto BusinessLocation);
...
public sealed record OrderLineDto(Guid ProductId, string Name, decimal Quantity, decimal UnitPrice, decimal TaxAmount, string? PreparationStationCode, decimal GstRate = 0, decimal CgstRate = 0, decimal SgstRate = 0);
