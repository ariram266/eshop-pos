...
import {
  getPreferredLocalDevRole,
  initializeAuth,
  isLocalDevelopment,
  LOCAL_DEV_ROLES,
  setPreferredLocalDevRole,
} from "./lib/auth";
...
export default function App() {
  ...
  const [mode, setMode] = useState<"pos" | "kds" | "catalog" | "operations">(
    "pos",
  );
  ...
  const load = async () => {
    try {
      ...
      const [currentActor, catalog] = await Promise.all([
        fetchActor(),
        fetchPosBootstrap(),
      ]);
  ...
  const products = useMemo(
    () =>
      (bootstrap?.products ?? []).filter(
        (product) =>
          product.active &&
          (!inventoryOnly || product.trackInventory) &&
          `${product.name} ${product.sku}`
            .toLowerCase()
            .includes(query.toLowerCase()),
      ),
    [bootstrap, inventoryOnly, query],
  );
  ...
  const tax = cart.reduce(
    (sum, line) =>
      ...
      (line.product.price *
        line.quantity *
        (line.product.gstRate || line.product.taxRate)) /
        100,
    0,
  );
  ...
  const addToCart = (product: Product) =>
    setCart((current) => {
      ...
      if (
        ...
        (existing?.quantity ?? 0) >= product.availableQuantity
      ) {
        setNotice(`${product.name} has insufficient stock`);
        return current;
      }
      return existing
        ? current.map((line) =>
        ...
        .map((line) =>
          line.product.id === productId
            ? {
                ...
                quantity: Math.max(
                  0,
                  Number((line.quantity + delta).toFixed(3)),
                ),
              }
            : line,
        )
  ...
  const saveCategory = async (event: React.FormEvent) => {
    ...
    try {
      if (editingCategoryId) {
        ...
        await createCategory(categoryName, categoryParentId || undefined);
  ...
  const addProduct = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setNotice(
        caught instanceof Error ? caught.message : "Product could not be added",
      );
    }
  };
  const saveProduct = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!editingProductId) return addProduct(event);
    try {
    ...
    return (
      <main className="login-screen">
        <section className="login-card">
          ...
          <p className="eyebrow">Counterpoint cloud POS</p>
          <h1>Loading secure workspace</h1>
          <p className="login-copy">
            Resolving your organization, location, catalog, and permissions.
          </p>
        </section>
      </main>
    );
    ...
    return (
      <main className="login-screen">
        <section className="login-card">
          ...
          <p className="login-copy">
            {isLocalDevelopment
              ? "Choose a local role for the guarded Development-only actor. No Microsoft Entra login is required locally."
              : "Use your Microsoft Entra account. POS transactions require a live connection to the cloud service."}
          </p>
          {error && <p className="form-error">{error}</p>}
          {isLocalDevelopment && (
            <label className="payment-select" style={{ marginBottom: "1rem" }}>
              Local role
              <select
                value={localRole}
                onChange={(event) => {
                  const nextRole =
                    event.target.value || getPreferredLocalDevRole();
                  setLocalRole(nextRole);
                  setPreferredLocalDevRole(nextRole);
                }}
              >
                <option value="">Select role</option>
                {LOCAL_DEV_ROLES.map((role) => (
          ...
          <button
            ...
            disabled={isLocalDevelopment && !localRole}
            onClick={() => {
              if (isLocalDevelopment) {
              ...
              void startEntraLogin().catch((caught) =>
                setError(
                  caught instanceof Error
                    ? caught.message
          ...
          >
            {isLocalDevelopment ? "Continue locally" : "Sign in with Microsoft"}
          </button>
        </section>
      </main>
    );
  ...
  return (
    <div className="operations-app">
      ...
      <main className="operations-main phase1-main">
        ...
        <div className="report-tabs primary-menu">
          <button
          ...
          {(actor.role === "OrganizationOwner" ||
            actor.role === "OperationsManager" ||
            actor.role === "StoreManager") && (
            <>
              <button
            ...
            <div className="phase1-layout">
              <section>
                ...
                <div className="catalog-filters">
                  ...
                  />
                  <select
                    aria-label="Filter catalog by inventory"
                    value={inventoryOnly ? "tracked" : ""}
                    onChange={(event) =>
                      setInventoryOnly(event.target.value === "tracked")
                    }
                  >
                ...
                <div className="real-product-grid">
                  {products.map((product) => (
                    ...
                    >
                      ...
                      <small>
                        {!product.trackInventory
                          ? "Prepared / no stock tracking"
                          : `${money(product.price)} · ${product.availableQuantity} ${product.unit}`}
                      </small>
                    </button>
                  ))}
                </div>
              ...
              <aside className="sale-panel">
                ...
                <div className="sale-lines">
                  {cart.map((line) => (
                    <div className="sale-line" key={line.product.id}>
                      <div>
      ...
      {lastOrder && (
        <div className="receipt-modal-backdrop">
          <section className="receipt-modal">
            ...
            <p>Payment recorded by the cloud service.</p>
            <div>
              <button
                className="primary-button"
                onClick={() => printReceipt(lastOrder, bootstrap?.businessLocation)}
              >
                Print receipt
              </button>
              <button
...
function KdsView({
  ...
  return (
    <section className="kds-screen">
      <div className="panel-heading">
        <div>
          <p className="eyebrow">Persistent SQL work queue</p>
      ...
      {items.length ? (
        <div className="kds-grid">
          {items.map((item) => (
            <article className="kds-item" key={item.id}>
              <span>
                {item.stationCode} · {item.orderNumber}
              </span>
              ...
              {nextStatus[item.status] && (
                <button
                  className="primary-button"
                  onClick={() =>
                    void onStatus(item.id, nextStatus[item.status]!)
                  }
                >
                  {nextStatus[item.status]}
                </button>
        ...
        <div className="empty-panel">
          No active preparation work. Refresh recovery state from SQL.
        </div>
      )}
    </section>
  );
